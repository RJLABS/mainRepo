...
IPAddress ip(192, 168, 101, 177); //RONI
IPAddress gateway(192,168,101,1);
IPAddress subnet(255,255,255,0);
String incomingData; //roni

boolean debug = false;  // true = more messages
//boolean debug = true;

// LED is needed for failure signalling
const short int BUILTIN_LED2 = 2;  //GPIO16 on NodeMCU (ESP-12)
...
void connectWifi() {
  ...
  WiFi.config(ip,gateway,subnet); //RONI
...
void loop() {
  ...
  if (telnetServer.hasClient()) {
   ...
   if(serverClient.available()>0){
    incomingData = serverClient.readStringUntil('\n');
    subProses();
